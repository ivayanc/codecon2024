...
from bot.utils.keyboards import EvacuationKeyboards, MainKeyboards
...
@evacuation_router.callback_query(F.data == 'for_me')
...
async def process_any_special_needs(call: CallbackQuery, state: FSMContext) -> None:
    if call.data == 'yes':
        ...
        await finish_request(call.message, state)
